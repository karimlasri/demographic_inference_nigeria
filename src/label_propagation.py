...
def update_scores(scores_matrix, norm_adj_matrix, init_matrix, alpha=0.5):
    ...
    scores_matrix = (
        alpha * csr_matrix.dot(norm_adj_matrix, scores_matrix)
        + (1 - alpha) * init_matrix
    )
...
def get_predictions(scores_matrix, matrix_indices, attr_classes):
    ...
    predictions = [
        attr_classes[p] for p in scores_matrix[matrix_indices, :].argmax(axis=1)
    ]
    return predictions
...
def get_test_matrix_indices(labeled_profiles, target_nodes):
    ...
    inverse_index = {
        row["user_id"]: matrix_idx for matrix_idx, row in target_nodes.iterrows()
    }
    test_matrix_indices = []
...
def get_test_indices_for_attr(
    test_matrix_indices, labeled_profiles, attr, attr_classes
):
    """Get test indices in adjacency matrix and in labeled profiles set for current attribute."""
    attr_test_matrix_indices = [
        idx
        for i, idx in enumerate(test_matrix_indices)
        if idx is not None and labeled_profiles.iloc[i][attr] in attr_classes
    ]
    attr_test_profile_indices = [
        i
        for i, idx in enumerate(test_matrix_indices)
        if idx is not None and labeled_profiles.iloc[i][attr] in attr_classes
    ]
...
def evaluate(
    predictions, ground_truth, scores_matrix, matrix_indices, labeled_profiles
):
    ...
    cov = (
        len([idx for idx in matrix_indices if scores_matrix[idx].sum() > 0])
        / labeled_profiles.shape[0]
    )
    return acc, cov
...
def get_eval_by_connections(
    ...
    eval_by_connections_params["max_connections"]
    eval_by_connections_params["bin_size"]
    for min_connections in np.arange(0, 500, 10):
        predictions = [
            attr_classes[p]
            for p in scores_matrix[
                [
                    idx
                    for i, idx in enumerate(test_matrix_indices)
                    if idx is not None
                    and n_connections[i] <= min_connections + 10
                    and labeled_profiles.iloc[i][attr] in attr_classes
                ],
                :,
            ].argmax(axis=1)
        ]
        ...
        coverage = (
            len(
                [
                    f
                    for i, f in enumerate(n_connections)
                    if f >= min_connections
                    and labeled_profiles.iloc[i][attr] in attr_classes
                ]
            )
            / labeled_profiles[labeled_profiles[attr].isin(attr_classes)].shape[0]
        )
        coverages.append(coverage)
    return accuracies, coverages


def plot_by_connections_threshold(
    accuracies, coverages, attr, eval_by_connections_params
):
    ...
    fig, ax = plt.subplots()
    ax.plot(
        np.arange(min_connections, max_connections, bin_size),
        accuracies,
        color="orange",
        marker="o",
    )
    ...
    if nm_results:
        ax2.axhline(
            y=nm_results[attr]["acc"],
            color="orange",
            linestyle="--",
            label="Name Matching Accuracy",
        )
        ax2.axhline(
            y=nm_results[attr]["cov"],
            color="blue",
            linestyle="--",
            label="Name Matching Coverage",
        )
        ax2.axhline(
            y=nm_results[attr]["maj"],
            color="black",
            linestyle="--",
...
def initialize_matrices_for_lp(
    nm_scores_df, attr, attr_classes, keep_init, norm_adj_matrix
):
    """Initialize scores and adjacency matrices prior to performing label propagation for current attribute."""
    init_df = nm_scores_df[
        ["{}_name_predict_{}".format(attr, attr_class) for attr_class in attr_classes]
    ]
    ...
    if keep_init:
        ...
        keep_vector = np.array(
            [1 if (row.sum() == 0) else 0 for row in scores_matrix]
        ).reshape(-1, 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name_matched_profiles",
        type=str,
        help="Path to the user profiles with name matching scores, i.e. the output of the name matching script.",
    )
    parser.add_argument(
        "--followership_path", type=str, help="Path to the followership data."
    )
    ...
    parser.add_argument(
        "--annotations_path", type=str, help="Path to labeled profiles."
    )
    parser.add_argument(
        "--symmetrize",
        type=bool,
        help="Whether the followership matrix should be made symmetric or not",
        default=True,
    )
    parser.add_argument(
        "--plot_by_connections",
        type=bool,
        help="Whether the evaluation scores should be plotted against the number of connections.",
        default=True,
    )
    ...
    ## Files loading
    name_matched_profiles, labeled_profiles = load_profiles_for_lp(
        args.name_matched_profiles, NAMES_TO_ATTRIBUTES_MAP, args.annotations_path
    )

    ## Load followership data
    norm_adj_matrix, target_nodes = load_followership_for_lp(
        args.followership_path, name_matched_profiles, args.symmetrize
    )
    ...
    for attr, attr_classes in CLASSES.items():
        ...
        init_matrix, scores_matrix, norm_adj_matrix = initialize_matrices_for_lp(
            nm_scores_df, attr, attr_classes, keep_init, norm_adj_matrix
        )

        # Get indices of test users for current attribute and their ground truth
        attr_test_matrix_indices, attr_test_profile_indices = get_test_indices_for_attr(
            test_matrix_indices, labeled_profiles, attr, attr_classes
        )
        ...
        for iteration in range(num_iterations):
            scores_matrix, predictions = perform_lp_iteration(
                scores_matrix,
                norm_adj_matrix,
                init_matrix,
                alpha,
        ...
        pkl.dump(
            predictions,
            open(
                "{}/lab_prop_predictions_{}_{}.npz".format(SCORES_PATH, attr, alpha),
                "wb",
            ),
        )
        np.save(
            "{}/lab_prop_scores_matrix_{}_{}.npz".format(SCORES_PATH, attr, alpha),
            scores_matrix,
        )

        if args.plot_by_connections:
            accuracies, coverages = get_eval_by_connections(
                labeled_profiles,
                attr,
                attr_classes,
                test_matrix_indices,
